---
title: "About"

name: "Dr. Praveen Sathiyamoorthi"

designation: "Assistant Professor"

department: "Department of Metallurgical Engineering"

institution: "Indian Institute of Technology (BHU) Varanasi"

photo: "/images/team/praveen.jpg"

email: "praveen.met@iitbhu.ac.in"

office: "Department of Metallurgical Engineering, IIT (BHU) Varanasi"

profiles:

  - title: "Google Scholar"
    url: "https://scholar.google.com/citations?user=v745JdMAAAAJ&hl=en"

  - title: "ORCID"
    url: "https://orcid.org/0000-0001-8803-3509"

  - title: "Scopus"
    url: "https://www.scopus.com/authid/detail.uri?authorId=57204760648"

  - title: "ResearchGate"
    url: "https://www.researchgate.net/profile/Praveen-Sathiyamoorthi"

stats:

  - value: "51+"
    label: "Journal Publications"

  - value: "37"
    label: "H-index"

  - value: "2"
    label: "Book Chapters"

  - value: "9"
    label: "Invited Talks"

quick_facts:

  - label: "Joined IIT (BHU)"
    value: "2022"

  - label: "Education"
    value: "M.S. & Ph.D., IIT Madras"

  - label: "Previous Institution"
    value: "POSTECH, South Korea"

  - label: "Research Focus"
    value: "Compositionally Complex Alloys"

  - label: "Research Approach"
    value: "Computational Alloy Design + Advanced Processing"

research_interests:

  - title: "Compositionally Complex Alloys"

    description: >
      Designing medium- and high-entropy alloys with superior
      mechanical performance through composition optimization,
      microstructural engineering, and thermodynamic principles.

  - title: "Computational Alloy Design"

    description: >
      CALPHAD-guided alloy development, thermodynamic modelling,
      and integrated computational materials engineering (ICME)
      for accelerated alloy discovery.

  - title: "Advanced Manufacturing"

    description: >
      Additive manufacturing, powder metallurgy,
      severe plastic deformation, and innovative processing
      routes for advanced metallic materials.

  - title: "Mechanical Behaviour"

    description: >
      Understanding deformation mechanisms, strengthening,
      cryogenic performance, ductility, and
      processing–structure–property relationships.

career:

  - year: "2022–Present"

    title: "Assistant Professor"

    organization: "Indian Institute of Technology (BHU) Varanasi"

  - year: "2021–2022"

    title: "Research Associate Professor"

    organization: "POSTECH, South Korea"

  - year: "2019–2021"

    title: "Research Assistant Professor"

    organization: "POSTECH, South Korea"

  - year: "2017–2019"

    title: "Postdoctoral Research Fellow"

    organization: "POSTECH, South Korea"

  - year: "2009–2016"

    title: "M.S. & Ph.D."

    organization: "Indian Institute of Technology Madras"

  - year: "2005–2009"

    title: "B.E. Metallurgical Engineering"

    organization: "Government College of Engineering, Salem"

service:

  - category: "Editorial Roles"

    items:

      - "Editorial Board Member – Scientific Reports"

      - "Editorial Board Member – Metals and Materials International"

      - "Associate Editor – Frontiers in Metals and Alloys"

      - "Guest Editor – Metals"

      - "Guest Editor – Frontiers in Materials"

awards:

  - category: "Research Awards"

    items:

      - "Institute Research Award, IIT Madras (2015)"

  - category: "Publication Awards"

    items:

      - "Best Article Award (2018)"

      - "Topmost Cited Article Award (2018–2019)"

  - category: "Presentation Awards"

    items:

      - "Best Poster Presentation"

      - "Third Prize – Oral Presentation (2015)"

      - "Third Prize – Oral Presentation (2013)"

cv: "/files/CV.pdf"

---

Dr. Praveen Sathiyamoorthi is an Assistant Professor in the Department of Metallurgical Engineering at the Indian Institute of Technology (BHU) Varanasi. His research focuses on the design, processing, and mechanical behaviour of compositionally complex alloys, including medium- and high-entropy alloys, for demanding structural applications.

His research integrates computational alloy design, thermodynamic modelling, advanced processing techniques, and detailed microstructural characterization to establish processing–structure–property relationships. Through this multidisciplinary approach, his group aims to accelerate the development of next-generation metallic materials with improved strength, ductility, damage tolerance, and cryogenic performance.

Prior to joining IIT (BHU) in 2022, he served in several research positions at Pohang University of Science and Technology (POSTECH), South Korea, where he worked extensively on advanced structural alloys and deformation mechanisms. He obtained his M.S. and Ph.D. (Dual Degree) in Metallurgical and Materials Engineering from IIT Madras. His work has resulted in numerous high-impact journal publications, invited talks, book chapters, and editorial contributions to leading international journals in materials science.